...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      addToast(t('enterEmailPassword'), 'error');
      return;
    }
    setLoading(true);
    ...
    if (error) {
      ...
      addToast(t('welcomeBackMsg'), 'success');
      navigate(redirectTo);
    }
  };
    ...
    <div className="min-h-screen bg-cream-100 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <h1 className="font-display font-bold text-2xl text-espresso-600">{t('welcomeBack')}</h1>
          <p className="text-sm text-espresso-300 mt-1">{t('loginToAccount')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-4">
          ...
          <div className="flex justify-between text-sm">
            <Link to="/register" className="text-sage-500 font-medium hover:underline">{t('createAccount')}</Link>
            <Link to="/" className="text-espresso-300 hover:underline">{t('browseMenu')}</Link>
          </div>
        </form>
      </div>
    </div>
