...
export function MemberDetailModal({ member, isAdmin, isMainKitchen, onClose, onDeleted }: { member: Member; isAdmin: boolean; isMainKitchen: boolean; onClose: () => void; onDeleted?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-espresso-900/50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      ...
      {showManualTx && (
        <ManualTransactionModalWrapper member={member} isMainKitchen={isMainKitchen} onClose={() => setShowManualTx(false)} onSaved={() => { setShowManualTx(false); loadData(txPage); }} />
      )}
    </div>
  );
}

function ManualTransactionModalWrapper({ member, isMainKitchen, onClose, onSaved }: { member: Member; isMainKitchen: boolean; onClose: () => void; onSaved: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] bg-espresso-900/50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-cream-100 rounded-2xl p-6 max-w-md w-full max-h-[85vh] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()}>
        <ManualTransactionModalInner member={member} isMainKitchen={isMainKitchen} onClose={onClose} onSaved={onSaved} />
      </div>
    </div>
  );
}

function ManualTransactionModalInner({ member, isMainKitchen, onClose, onSaved }: { member: Member; isMainKitchen: boolean; onClose: () => void; onSaved: () => void }) {
  const { addToast } = useToast();
  const { session } = useAuth();
  ...
  const handleSave = async () => {
    ...
    if (isMainKitchen && !file) {
      addToast('Main Kitchen staff must upload a receipt to scan the amount', 'error');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <>
      ...
      <div className="space-y-3">
        <div>
          <label className="text-sm font-medium text-espresso-500 mb-1 block">Receipt Image {isMainKitchen ? '(required — scan to set amount)' : '(optional — auto-scans amount)'}</label>
            ...
            <div className="border-2 border-dashed border-cream-300 rounded-xl p-4 text-center cursor-pointer hover:border-sage-400 transition-colors">
              {file ? (
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-sm text-espresso-400">{scanning ? 'Scanning receipt...' : isMainKitchen ? 'Upload receipt — amount is filled by scan' : 'Tap to upload receipt for OCR'}</p>
                </div>
              )}
            </div>
            <input
        ...
        <div>
          <label className="text-sm font-medium text-espresso-500 mb-1 block">Amount (Rp){isMainKitchen && ' — set by receipt scan'}</label>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-espresso-300">Rp</span>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} disabled={isMainKitchen} readOnly={isMainKitchen} className="input-field pl-8 text-sm py-2.5" placeholder="0" />
          </div>
      ...
      <button onClick={handleSave} disabled={saving || scanning || (isMainKitchen && !file)} className="btn-primary w-full mt-4 flex items-center justify-center gap-2">
